...
1. Implement [[#inline][Inlining of small objects]]: This seems to be an obvious optimization. Even if the estimated performance gains from decreased I/O might be low we can expect more performance gains by doing larger reads (see [[#batching-reads][Batching Reads]]), improved CPU efficiency while decoding content (see [[#cpu-efficiency][CPU efficiency]]) as well a more shallow tree (see [[#optimize-tree-descent][Optimize tree descent]]).
