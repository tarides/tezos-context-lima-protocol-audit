...
- Remove limitations on the number of entries that the ~Dict~ can hold (see [[https://github.com/metanivek/irmin/commit/92d910b72b5f7140560c02d0d8d28728f95c39a0][metanivek/irmin-92d910b]]). Path segments are always added to the dictionary before writing.
 ...
 |                          | 3.7.1   | 500k-dict      | infinite-dict  |
 |--------------------------+---------+----------------+----------------|
 | CPU time elapsed         | 104m07s | 109m42s (105%) | 130m36s (125%) |
 | total bytes read         | 66.466G | 64.976G (98%)  | 60.747G (91%)  |
 | total bytes written      | 46.360G | 41.671G (90%)  | 37.903G (82%)  |
 | max memory usage (bytes) | 0.394G  | 0.508G (129%)  | 2.534G (643%)  |

The amount of bytes read and written decreases as improved usage of the ~Dict~ results in more compact representations. Memory usage increases as expected. Unfortunately it seems that overall performance degrades.

Performance decrease could be due to the in-memory Dict (a ~Hashtbl~) being taken to it's limits. A more efficient in-memory datastructure might offset this and result in performance increase.
...
Concretely we can imagine a scheme where all content that is smaller than the size of the has is inlined. Based on observations around this would be about 80% of the content in the Tezos Context Store. We can expect considerable optimization in disk usage and hopefully also overall performance.
