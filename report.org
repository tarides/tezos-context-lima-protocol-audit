...
The performance charateristics of ~irmin-pack~ (and Irmin) seems to be closer to a file-system than a key-value store, whereas the way it is used from Tezos seems to be more like a key-value store.
